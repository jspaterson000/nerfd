import { registerModelDeclarations } from '@nerfd/core';
  ...
  for (const adapter of targets) {
    ...
    try {
      for (const s of adapter.backfill!(since)) {
        if (getSession(s.id)) { skipped++; continue; }
        putSession(s);
        added++;
      }
    } catch (e) {
    ...
    process.stdout.write(`${adapter.label.padEnd(12)} ${added} added, ${skipped} already recorded (since ${since.slice(0, 10)})\n`);
  }
