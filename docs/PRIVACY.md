...
| `reporter_id` | rotating hash of your install id **and the ISO week**. Deduplicates and rate-limits one person's sessions within a week. Not an account, not derived from anything about you. Because it rotates, counting distinct ids counts **reporter-weeks**, not people: one person contributes a new id every week, by design, so that sessions cannot be linked across weeks. The board says "reporter-weeks" for that number rather than overstating its reach. |
...
| `plan_source` | one word: `declared` (you ran `nerfd plan`), `detected` (read off the tool's own config on this machine), `assumed` (imported history: the session ended inside a subscription period this machine can prove, so the tool's current plan was applied to it), or `unknown`. The file and field a detector opened stay local; see "What the plan detector opens" below. That word and `plan_id` are the entire public surface of detection, and `assumed` is published precisely so a weaker answer can be told apart from a firm one and dropped. |
...
| `duration_s` | the wall-clock span from the session's first turn to its last. A session resumed the next morning spans the night, which is why it is not what "hours" means anywhere in the report. |
| `metrics` | counts only: prompts, turns, tool calls, edits, files touched, tests run, errors, rate-limit hits (the subscription wall: 429 / quota), `overloaded` (the provider's fleet was busy: a 529, counted apart because it is not a quota wall), timeouts, model switches, interrupts, `tool_call_errors` (tool calls the model emitted that failed to parse or violated the schema — the number that differs most between hosts of the same weights), `context_limit_hits` (a compaction or a context-length error), token totals, latency p50/p95, `active_s` (time actually spent working, the gaps between turns with each idle gap capped at ten minutes - a duration, like the latencies), and the subscription-window usage percentage where the tool reports it. |
...
| `automated` | a boolean: true when nobody prompted this session. A Codex auto-review, a CI agent, a scripted run - no human prompts and no human turns. It is a fact about the session, not about you, and it exists so the board can keep unattended runs out of the quality rankings while still counting what they cost. |
...
- **The scoring formula is published** on the board footer and in `docs/METHOD.md`, so a number you disagree with can be argued with.
